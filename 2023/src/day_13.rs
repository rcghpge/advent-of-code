...
pub fn run() {
    // Read the input file
    let input_file = "data/input13.txt";
    let input = fs::read_to_string(input_file).expect("Failed to read input file");

    // Parse the grids from the input
    ...
    // Calculate and print results for each reflection error count
    for (part, num_reflection_errors) in [(1, 0), (2, 1)] {
        let total_reflection_ids: u32 = grids
            .iter()
            .map(|grid| find_reflection_id(grid, num_reflection_errors))
            .sum();
        println!("Part {}: {}", part, total_reflection_ids);
    }
}
