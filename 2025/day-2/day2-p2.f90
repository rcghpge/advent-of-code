program gift_shop_dbms2
  ...
  character(len=30) :: id_str, base_seq, test_str
  integer :: length, base_len, repeat_count, k
  ...
  do i = 1, count_ranges
     do j = ranges(i, 1), ranges(i, 2)
        ...
        do base_len = 1, length/2
           if (mod(length, base_len) == 0) then
              base_seq = id_str(1:base_len)

              if (base_seq(1:1) == '0') cycle
              
              repeat_count = length/base_len
              test_str = ''
              do k = 1, repeat_count
                 test_str = trim(test_str)//base_seq
              end do
              
              if (test_str == id_str) then
  ...
  print '(A,I11)', 'Sum of invalid product IDs (DBMS2): ', invalid_sum

end program gift_shop_dbms2
